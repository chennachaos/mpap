...
void TreeNode<3>::calcStiffnessAndResidualCutFEMFluid(MatrixXd& Klocal, VectorXd& Flocal, int domainCur)
{
    ...
    double  fact, fact2, b1, b2, b3, b4, b5, b6, b7, b8, acceFact, c1, CI=1.0;
    double  pres, Da, Db, af, am, muTaf, rad, urdr, urdr2, h, h2, dt, tau[3], beta[10];
...
void TreeNode<3>::calcStiffnessAndResidualCutFEMFluid(MatrixXd& Klocal, VectorXd& Flocal, int domainCur)
{
    ...
    double  JacTemp, Jac, dvol, stabParam;
    double  fact, fact2, b1, b2, b3, b4, b5, b6, b7, b8, acceFact, c1, CI=4.0;
    double  pres, Da, Db, af, am, muTaf, rad, urdr, urdr2, h, h2, dt, tau[3];
...
int TreeNode<3>::computeGaussPointsAdapIntegrationBoundary(int side, int refLev1, int refLev2, int inclFlag, int domainCur)
{
    ...
    //cout << " side pppppppppppppppp = " << side << endl;
    ...
    {
      for(ii=0; ii<3; ii++)
        BoundaryQuadrature[side].gausspoints[gp][ii] = (2.0*BoundaryQuadrature[side].gausspoints[gp][ii] - knots[ii][3])/knots[ii][2];
    }
        ...
        for(gp=0; gp<nGauss; gp++)
        {
            ...
            if(side == 3)
            {
              ...
              */
              ...
              //mu = elmDat[5];
              //res = specVal - computeValue2(dir, N);

              //grad(0,0) = computeValue2(0, dN_dx);
              //grad(0,1) = computeValue2(0, dN_dy);
              //grad(1,0) = computeValue2(1, dN_dx);
              //grad(1,1) = computeValue2(1, dN_dy);
              //pres      = computeValue2(2, N);
