...
SolidALE::SolidALE(void)                       
{                                                  
  addElemGrpProp(MATERIAL);

  // add new type
  
  DomainType *solidALE = domain.newType(SOLIDALE,FINITEELEMENTBVPWI);
  
  if (solidALE == NULL) return;  // domain type already exists

  solidALE->key.addNew("material", "control");
...
void SolidALE::readInputData(std::ifstream &Ifile, MyString &line)
{
  MyString *word;
 
  char fct[] = "SolidALE::readInputData";
  ...
  switch (domain[SOLID].key.whichBegins(line))
  {
    ...
    case  1: cout << "     SOLID: reading control ...\n\n";
	     ...
	     if (!word[1].toInt(&tis))      prgError(4,fct,"input error in 'control'!");
	     if (tis < 0) tis = 0;
             ...
             for (i=2; i<nn; i++) 
	       if (!word[i].toDbl(&(td[i-2]))) prgError(5,fct,"input error in 'control'!");
...
void SolidALE::prepareInputData(void)
{
  char fct[] = "SolidALE::prepareInputData";
  ...
  if (tol < -1.) prgError(1,"SolidALE::prepareInputData","'control' data undefined!");

  if (ndf < ndm) prgError(2,"SolidALE::prepareInputData","ndf < ndm not admissible!");
...
void SolidALE::prepareInteractions(void)
{
  // go and inherit from ancestors
...
void SolidALE::setTimeParam(void)
{
  ...
  switch (tis)
  {
    ...
    default: prgError(1,"SolidALE::setTimeParam","invalid value of tis!");
  }
  return;
}
...
void SolidALE::timeUpdate(void)
{
  ...
  switch (tis)
  {
    ...
    default: prgError(1,"SolidALE::timeUpdate","invalid value of tis!");
  }
...
void SolidALE::updateIterStep(void)
{
  ...
  switch (tis)  // note that changing this stuff requires
	        // adjusting of ElementSolidALE::diffStiffTest,
		// otherwise, 'diff' command for elements is in trouble
  {
    ...
    default: prgError(1,"SolidALE::updateIterStep","invalid value of tis!");
  }
...
int SolidALE::calcStiffnessAndResidual(int printRes, bool zeroMtx, bool zeroRes)
{
  ...
  return FiniteElementBVPWI::calcStiffnessAndResidual(printRes, zeroMtx, zeroRes);
}
