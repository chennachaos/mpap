CDOC BEGIN_SUBROUTINE CSTOGD
CDOC Spatial elasticity tensor for the Ogden hyperelastic material model
CDOC
CDOC This routine computes the spatial elasticity tensor, a, for the
CDOC regularised Ogden hyperelastic material model under plane stress,
CDOC plane strain and axisymmetric conditions. Note that under plane
CDOC stress the model implemented here is exactly incompressible,
CDOC whereas under plane strain and axisymmetric conditions the
CDOC regularised (penalty type) approach is used to enforce
CDOC incompressibility. The (Cauchy) stress tensor components of the
CDOC Ogden hyperelastic model under plane stress/strain and axisymmetric
CDOC states are evaluated in subroutine \smparm{SUOGD}.
CDOC
CDOC BEGIN_PARAMETERS
CDOC DOUBLE_PRECISION AMATX  <  Matrix of components of the spatial
CDOC C                          elasticity (fourth order) tensor.
