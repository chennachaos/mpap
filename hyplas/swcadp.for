CDOC BEGIN_SUBROUTINE SWCADP
CDOC Initialise/switch state var. for the Capped Drucker-Prager model
CDOC
CDOC This initialises and switches state variables (between current and
CDOC previous values) for the Capped Drucker-Prager elasto-plastic
CDOC material model.
CDOC
...
C Arguments
     ...
     1     0.0D0,1.0D0/
C***********************************************************************
C INITIALISE/SWITCH DATA FOR THE CAPPED DRUCKER-PRAGER MATERIAL MODEL
C
...
C and the volumetric plastic strain
          IF(NTYPE.EQ.2.OR.NTYPE.EQ.3)THEN
            RSTAVC(1)=R1
            RSTAVC(2)=R1
            RSTAVC(3)=R0
