CDOC BEGIN_SUBROUTINE SUPDSC
CDOC State update for the planar double-slip single crystal model.
CDOC
CDOC This routine uses an implicit elastic predictor/return
CDOC mapping algorithm as the state update procedure for the
CDOC anisotropic finite strain planar double-slip single crystal model
CDOC with Taylor isotropic hardening. This model is compatible only
CDOC with the plane strain assumption.
CDOC It has two slip planes comprising a total of four slip directions
CDOC (two directions opposite to each other in each plane).
CDOC
...
CDOC C                          flag, \smparm{IFPLAS}, the state
CDOC C                          update failure flag, \smparm{SUFAIL} and
CDOC C                          the active slip-system flags,
CDOC C                          \smparm{S1ACT}, \smparm{S2ACT},
CDOC C                          \smparm{S3ACT} and \smparm{S4ACT}.
...
CDOC C                          for any reason.
CDOC C                          Each active slip-system flags is set to
CDOC C                          \smparm{.TRUE.} only if the
CDOC C                          corresponding system is currently
CDOC C                          active, i.e., \smparm{S1ACT} is set to
CDOC C                          \smparm{.TRUE.} if system 1 is active,
CDOC C                          \smparm{S2ACT} is set to
CDOC C                          \smparm{.TRUE.} if system 2 is active
CDOC C                          and so on.
CDOC C                          Each flag will be set to
CDOC C                          \smparm{.FALSE.} if the corresponding
CDOC C                          system is not currently active.
