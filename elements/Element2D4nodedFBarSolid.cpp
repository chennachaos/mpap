...
Element2D4nodedFBarSolid::Element2D4nodedFBarSolid(void)
{
  return;
}
...
Element2D4nodedFBarSolid::~Element2D4nodedFBarSolid()
{
  return;
}
...
bool Element2D4nodedFBarSolid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element2D4nodedFBarSolid::stressStrainState(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 3)

    prgError(1,"Element2D4nodedFBarSolid::stressStrainState","element type data empty!");
...
int Element2D4nodedFBarSolid::finiteStrain(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 2)

    prgError(1,"Element2D4nodedFBarSolid::finiteStrain","element type data empty!");
...
int Element2D4nodedFBarSolid::nGaussPoints(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 1)

    prgError(1,"Element2D4nodedFBarSolid::nGaussPoints","element type data empty!");
	
  int ngp = roundToInt(((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data[0]);

  if (ngp!=1 && ngp!=4 && ngp!=9) 
	  
    prgError(2,"Element2D4nodedFBarSolid::nGaussPoints","invalid number of Gauss points!");
...
int Element2D4nodedFBarSolid::calcStiffnessAndResidual(void)
{
  char fct[] = "Element2D4nodedFBarSolid::calcStiffnessAndResidual";
  ...
  if (eG->dom->tis != 0) 
  {
    ...
    else prgError(1,fct,"mass lumping not available yet!");
  }
  ...
  element2dfbarsolid_(elmDat,matDat,timDat,xl,ul,intVar1,intVar2,s,p,
...
void Element2D4nodedFBarSolid::projectIntVar(int indx)
{
...
void Element2D4nodedFBarSolid::projectStress(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element2D4nodedFBarSolid::projectStress","nstu*nstu < ngp * 4!"); return; }
...
void Element2D4nodedFBarSolid::projectToNodes(double *dat, int indx, int nn)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  switch (ngp)
  {
    ...
    default: prgError(1,"Element2D4nodedFBarSolid::projectToNodes","invalid ngp");
  }
...
void Element2D4nodedFBarSolid::plotGaussPoints(int num, bool defFlg)
{
  ...
  if (ngp > 9) prgError(1,"Element2D4nodedFBarSolid::plotGaussPoints","ngp > 9 !");
