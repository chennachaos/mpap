...
Element2D8nodedQuadraticSolid::Element2D8nodedQuadraticSolid(void)
{
  return;
}
...
Element2D8nodedQuadraticSolid::~Element2D8nodedQuadraticSolid()
{
  return;
}
...
bool Element2D8nodedQuadraticSolid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element2D8nodedQuadraticSolid::stressStrainState(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 3)

    prgError(1,"Element2D8nodedQuadraticSolid::stressStrainState","element type data empty!");
...
int Element2D8nodedQuadraticSolid::finiteStrain(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 2)

    prgError(1,"Element2D8nodedQuadraticSolid::finiteStrain","element type data empty!");
...
int Element2D8nodedQuadraticSolid::nGaussPoints(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 1)

    prgError(1,"Element2D8nodedQuadraticSolid::nGaussPoints","element type data empty!");
	
  int ngp = roundToInt(((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data[0]);

  if (ngp!=1 && ngp!=4 && ngp!=9) 
	  
    prgError(2,"Element2D8nodedQuadraticSolid::nGaussPoints","invalid number of Gauss points!");
...
int Element2D8nodedQuadraticSolid::calcStiffnessAndResidual(void)
{
  ...
  if (eG->dom->tis != 0) 
  {
    ...
    else prgError(1,"Element2D8nodedQuadraticSolid::calcStiffnessAndResidual",
...
void Element2D8nodedQuadraticSolid::projectIntVar(int indx)
{
...
void Element2D8nodedQuadraticSolid::projectStress(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element2D8nodedQuadraticSolid::projectStress","nstu*nstu < ngp * 4!"); return; }
...
void Element2D8nodedQuadraticSolid::projectToNodes(double *dat, int indx, int nn)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nn < 1) 
    { COUT << "nn = 1!  Element2D8nodedQuadraticSolid::projectToNodes aborted!\n\n"; return; }
  ...
  switch (ngp)
  {
    ...
    default: prgError(1,"Element2D8nodedQuadraticSolid::projectToNodes","invalid ngp");
  }
...
void Element2D8nodedQuadraticSolid::plotGaussPoints(int num, bool defFlg)
{
  double d = (plot.dAct[0] + plot.dAct[1]) * .0055,

         X[16], xi[18], wgp[9], shp[24], detJ, xgp[3];
 
  int    l, i, ngp = nGaussPoints(), nen_ = 8;

  if (ngp > 9) prgError(1,"Element2D8nodedQuadraticSolid::plotGaussPoints","ngp > 9 !");

  if (ngp != GpDatId.n) return;
  
  if (defFlg) for (i=0; i<8; i++) { X[i+i] = x (i+1,1); X[i+i+1] = x (i+1,2); }
  else        for (i=0; i<8; i++) { X[i+i] = x0(i+1,1); X[i+i+1] = x0(i+1,2); }
  ...
  for (l=0; l<ngp; l++) 
  {
    ...
    xgp[0] = 0.; for (i=0; i<8; i++) xgp[0] += shp[i*3+2] * X[i+i];	    
    xgp[1] = 0.; for (i=0; i<8; i++) xgp[1] += shp[i*3+2] * X[i+i+1];	    
...
void Element2D8nodedQuadraticSolid::projectError(int)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element2D8nodedQuadraticSolid::projectError","nstu*nstu < ngp!"); return; }
