...
Element2D4nodedLinearSolid::Element2D4nodedLinearSolid(void)
{
  return;
}
...
Element2D4nodedLinearSolid::~Element2D4nodedLinearSolid()
{
  return;
}


bool Element2D4nodedLinearSolid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element2D4nodedLinearSolid::stressStrainState(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 3)

    prgError(1,"Element2D4nodedLinearSolid::stressStrainState","element type data empty!");
...
int Element2D4nodedLinearSolid::finiteStrain(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 2)

    prgError(1,"Element2D4nodedLinearSolid::finiteStrain","element type data empty!");
...
int Element2D4nodedLinearSolid::nGaussPoints(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 1)

    prgError(1,"Element2D4nodedLinearSolid::nGaussPoints","element type data empty!");
	
  int ngp = roundToInt(((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data[0]);

  if (ngp!=1 && ngp!=4 && ngp!=9) 
	  
    prgError(2,"Element2D4nodedLinearSolid::nGaussPoints","invalid number of Gauss points!");
...
int Element2D4nodedLinearSolid::calcStiffnessAndResidual(void)
{
  ...
  if (eG->dom->tis != 0) 
  {
    ...
    else prgError(1,"Element2D4nodedLinearSolid::calcStiffnessAndResidual",
  ...
  cout.precision(5);
  cout << "    Stiffness Matrix  FE " << endl;
  cout << endl;
  cout << endl;
  for(int ii=0;ii<nstu;ii++)
  {
    cout << '\t' ;
    for(int jj=0;jj<nstu;jj++)
    {
      cout  << s[ii*nstu+jj] << "   " ;
    }
    cout << endl;
...
void Element2D4nodedLinearSolid::projectIntVar(int indx)
{
...
void Element2D4nodedLinearSolid::projectStress(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element2D4nodedLinearSolid::projectStress","nstu*nstu < ngp * 4!"); return; }
...
void Element2D4nodedLinearSolid::projectError(int)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element2D4nodedLinearSolid::projectError","nstu*nstu < ngp * 4!"); return; }
...
void Element2D4nodedLinearSolid::projectToNodes(double *dat, int indx, int nn)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nn < 1) 
    { COUT << "nn = 1!  Element2D4nodedLinearSolid::projectToNodes aborted!\n\n"; return; }
  ...
  switch (ngp)
  {
    ...
    default: prgError(1,"Element2D4nodedLinearSolid::projectToNodes","invalid ngp");
  }
...
void Element2D4nodedLinearSolid::plotGaussPoints(int num, bool defFlg)
{
  ...
  if (ngp > 9) prgError(1,"Element2D4nodedLinearSolid::plotGaussPoints","ngp > 9 !");
