...
Element3D8nodedFBarSolid::Element3D8nodedFBarSolid(void)
{
  if (debug) cout << " constructor Element3D8nodedFBarSolid\n\n";
...
Element3D8nodedFBarSolid::~Element3D8nodedFBarSolid()
{
  if (debug) cout << " destructor Element3D8nodedFBarSolid\n\n";
...
bool Element3D8nodedFBarSolid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element3D8nodedFBarSolid::finiteStrain(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 2)

    prgError(1,"Element3D8nodedFBarSolid::finiteStrain","element type data empty!");
...
int Element3D8nodedFBarSolid::nGaussPoints(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 1)

    prgError(1,"Element3D8nodedFBarSolid::nGaussPoints","element type data empty!");
	
  int ngp = roundToInt(((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data[0]);

  if (ngp!=1 && ngp!=8 && ngp!=27) 
	  
    prgError(2,"Element3D8nodedFBarSolid::nGaussPoints","invalid number of Gauss points!");
...
int Element3D8nodedFBarSolid::calcStiffnessAndResidual(void)
{
  ...
  if (eG->dom->tis != 0) 
  {
    ...
    else prgError(1,"Element3D8nodedFBarSolid::calcStiffnessAndResidual",
  ...
  element3dfbarsolid_(elmDat,matDat,timDat,xl,ul,intVar1,intVar2,s,p,
...
void Element3D8nodedFBarSolid::projectIntVar(int indx)
{
...
void Element3D8nodedFBarSolid::projectStress(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element3D8nodedFBarSolid::projectStress","nstu*nstu < ngp * 4!"); return; }
  
  element3dfbarsolid_(elmDat,matDat,timDat,xl,ul,intVar1,intVar2,s,s,
	                  &ndf_,&ndm_,&nen_,&nelm,&nmat,&nivGP,&matId,&isw,
			  &errCode,(Element*) this);
...
void Element3D8nodedFBarSolid::projectToNodes(double *dat, int indx, int nn)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nn < 1) 
    { COUT << "nn = 1!  Element3D8nodedFBarSolid::projectToNodes aborted!\n\n"; return; }
  ...
  switch (ngp)
  {
    ...
    default: prgError(1,"Element3D8nodedFBarSolid::projectToNodes","invalid ngp");
  }
...
void Element3D8nodedFBarSolid::plotGaussPoints(int num, bool defFlg)
{
  ...
  if (ngp > 9) prgError(1,"Element3D8nodedFBarSolid::plotGaussPoints","ngp > 9 !");
