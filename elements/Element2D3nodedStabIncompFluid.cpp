...
Element2D3nodedStabIncompFluid::Element2D3nodedStabIncompFluid(void)
{
  if (debug) cout << " constructor Element2D3nodedStabIncompFluid\n\n";
...
Element2D3nodedStabIncompFluid::~Element2D3nodedStabIncompFluid()
{
  if (debug) cout << " destructor Element2D3nodedStabIncompFluid\n\n";
...
bool Element2D3nodedStabIncompFluid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element2D3nodedStabIncompFluid::calcStiffnessAndResidual(void)
{
  ...
  double *s      = eG->dom->s,
         ...
         *xl     = eG->dom->xl,
         *ul     = eG->dom->ul,
         *elmDat = eG->elemProp[ELEMENTTYPE]->data.x,
         *timDat = eG->dom->td,
         *dt     = &(mpapTime.dt);
  ...
  element2dstabincompfluid_(elmDat,timDat,xl,ul,dt,s,p,&ndf_,&ndm_,&nen_,&nelm,&isw);
...
void Element2D3nodedStabIncompFluid::projectError(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
...
int Element2D3nodedStabIncompFluid::calcMeshDerivatives(void)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
...
void Element2D3nodedStabIncompFluid::diffMeshDerivTest(double ddd, int dig, int dig2, bool gfrmt)
{
  int    i, jnd, jj, j, k, 
         nstu = nen()*ndf(), 
         nstx = nen()*ndm(), 
