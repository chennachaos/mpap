...
Element3D8nodedLinearSolid::Element3D8nodedLinearSolid(void)
{
  if (debug) cout << " constructor Element3D8nodedLinearSolid\n\n";
...
Element3D8nodedLinearSolid::~Element3D8nodedLinearSolid()
{
  if (debug) cout << " destructor Element3D8nodedLinearSolid\n\n";
...
bool Element3D8nodedLinearSolid::forDomainType(int domType)
{
  switch (domType)
  {
...
int Element3D8nodedLinearSolid::finiteStrain(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 2)

    prgError(1,"Element3D8nodedLinearSolid::finiteStrain","element type data empty!");
...
int Element3D8nodedLinearSolid::nGaussPoints(void)
{
  if (((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data.n < 1)

    prgError(1,"Element3D8nodedLinearSolid::nGaussPoints","element type data empty!");
	
  int ngp = roundToInt(((ElementGroup*)elemGrp)->elemProp[ELEMENTTYPE]->data[0]);

  if (ngp!=1 && ngp!=8 && ngp!=27) 
	  
    prgError(2,"Element3D8nodedLinearSolid::nGaussPoints","invalid number of Gauss points!");
...
int Element3D8nodedLinearSolid::calcStiffnessAndResidual(void)
{
  ...
  if (eG->dom->tis != 0) 
  {
    ...
    else prgError(1,"Element3D8nodedLinearSolid::calcStiffnessAndResidual",
...
void Element3D8nodedLinearSolid::projectIntVar(int indx)
{
...
void Element3D8nodedLinearSolid::projectStress(int indx)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nstu*nstu < ngp * 4) 
    { prgWarning(1,"Element3D8nodedLinearSolid::projectStress","nstu*nstu < ngp * 4!"); return; }
...
void Element3D8nodedLinearSolid::projectToNodes(double *dat, int indx, int nn)
{
  ElementGroup *eG = (ElementGroup*) elemGrp;
  ...
  if (nn < 1) 
    { COUT << "nn = 1!  Element3D8nodedLinearSolid::projectToNodes aborted!\n\n"; return; }
  ...
  switch (ngp)
  {
    ...
    default: prgError(1,"Element3D8nodedLinearSolid::projectToNodes","invalid ngp");
  }
...
void Element3D8nodedLinearSolid::plotGaussPoints(int num, bool defFlg)
{
  ...
  if (ngp > 9) prgError(1,"Element3D8nodedLinearSolid::plotGaussPoints","ngp > 9 !");
